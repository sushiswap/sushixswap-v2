...
contract StargateAdapterWithFee is ISushiXSwapV2Adapter, IStargateReceiver, Ownable {
    ...
    function setFee(uint8 _adapterFee) public onlyOwner {
        // 100 bps or 1% is the highest fee can be set
        require(
            (_adapterFee <= 100)
        ); 
        adapterFee = _adapterFee;
    }
    ...
    function adapterBridge(
        ...
        uint256 feeToTake = (params.amount * adapterFee) / 10000;
        uint256 amountToBridge = params.amount - feeToTake;
        ...
        IERC20(params.token).safeTransfer(feeCollector, feeToTake);
